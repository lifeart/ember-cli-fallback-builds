...
class BuildTask extends Task {
  ...
  run({ outputPath, environment }) {
    ...
    const legacyResult = spawn(`ember`, ["build:fallback-programmatic"], {
      env: legacyEnv,
    ...
    if (legacyResult.status !== 0) {
      ui.writeLine(legacyResult.output.join(''));
    }

    const modernResult = spawn(`ember`, ["build:fallback-programmatic"], {
      env: modernEnv,
      cwd: this.project.root,
      encoding: "utf8",
      shell: true
    ...
    if (modernResult.status !== 0) {
      ui.writeLine(modernResult.output.join(''));
    }
